...
function ContributorDashboardPage() {
  const targetUnlockId = new URLSearchParams(window.location.search).get('targetUnlockId')
  ...
  useEffect(() => {
    ...
    fetchMyDocuments({ limit: 50 }).then((response) => setDocuments(response.data || [])).catch((error) => setNotice(error.response?.data?.error?.message || 'Không thể tải danh sách tài liệu.')).finally(() => setLoading(false))
  }, [])

  async function handleSaveEdit(data) {
    if (!editingDocument) return
    const res = await updateMyDocument(editingDocument.id, data)
    setDocuments((prev) => prev.map((d) => (d.id === editingDocument.id ? { ...d, ...res.data } : d)))
    setNotice('Đã cập nhật thông tin tài liệu thành công.')
  }
  ...
  async function handleDeleteDocument(docId) {
    ...
    try {
      ...
      setNotice('Đã xóa tài liệu thành công.')
    } catch (err) {
      setNotice(err.response?.data?.error?.message || 'Không thể xóa tài liệu.')
    }
  }
  ...
  function changeShareMode(event) {
    const val = event.target.value
    if (val === 'LOCKED') {
      setForm((current) => ({ ...current, visibility: 'PUBLIC', isLocked: true }))
    } else {
      setForm((current) => ({ ...current, visibility: val, isLocked: false }))
    }
  }
  ...
  async function submit(event) {
    ...
    Object.entries(form).filter(([key]) => !key.endsWith('Query') && key !== 'isLocked').forEach(([key, value]) => { if (value) data.append(key, value) })
    data.append('isLocked', String(Boolean(form.isLocked)))
    ...
    if (targetUnlockId) {
      data.append('targetUnlockId', targetUnlockId)
    }
    ...
    try {
      ...
      setNotice('Tải lên thành công! Bạn được cộng +2 lượt tải vào tài khoản.')
      if (targetUnlockId) setUnlockedSuccess(true)
      clearForm()
    } catch (error) {
  ...
  return <div className="min-h-screen bg-[#f9f9ff]"><div className="mx-auto flex max-w-7xl flex-col gap-10 px-4 py-8 lg:px-6 lg:py-10">
    ...
    {targetUnlockId && (
      <div className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-900 shadow-xs">
        <div className="flex items-center gap-2 text-sm font-bold">
          <span className="text-xl">🎯</span>
          <span>Mô hình &quot;Cho đi để nhận lại&quot; (Học liệu số Give-to-Get)</span>
        </div>
        <p className="mt-1 text-xs leading-relaxed text-amber-800">
          Bạn đang đóng góp tài liệu học tập để nhận ngay <strong>+2 Lượt tải (Credits)</strong> và hệ thống sẽ tự động mở khóa tài liệu bạn đang cần!
        </p>
      </div>
    )}

    {unlockedSuccess && targetUnlockId && (
      <div className="flex items-center justify-between rounded-xl border border-emerald-300 bg-emerald-50 p-4 text-emerald-900 shadow-sm">
        <div>
          <h4 className="text-sm font-bold">🎉 Chúc mừng bạn đã mở khóa thành công!</h4>
          <p className="mt-0.5 text-xs text-emerald-800">
            Tài liệu bạn đang cần đã được tự động mở khóa. Bạn có thể quay lại đọc trọn vẹn và tải file ngay bây giờ.
          </p>
        </div>
        <Link
          className="rounded-lg bg-emerald-700 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-emerald-800"
          to={`/tai-lieu/${targetUnlockId}`}
        >
          Xem tài liệu đã mở khóa →
        </Link>
      </div>
    )}
...
function DocumentTable({ apiOrigin, documents, loading, onEditDocument, onDeleteDocument }) {
  return <section><div className="mb-4 flex flex-col justify-between gap-3 md:flex-row md:items-end"><div><h2 className="text-xl font-bold text-[#141b2b]">Danh sách học liệu của tôi</h2><p className="mt-1 text-xs text-[#444653]">Quản lý trực quan, theo dõi kiểm duyệt và cập nhật phiên bản mới cho tài liệu</p></div><div className="flex gap-1 rounded-lg bg-[#f1f3ff] p-1 text-[11px]"><span className="rounded bg-white px-3 py-2 font-bold text-[#00288e] shadow-sm">Tất cả ({documents.length})</span><span className="px-3 py-2 text-[#444653]">Đã xuất bản</span><span className="px-3 py-2 text-[#444653]">Chờ xử lý AI</span></div></div><div className="overflow-x-auto rounded-xl bg-white shadow-sm"><table className="w-full min-w-195 border-collapse text-left text-xs"><thead className="bg-[#f1f3ff] text-[10px] uppercase tracking-wider text-[#444653]"><tr><th className="p-4">Tên tài liệu &amp; Mã tệp</th><th className="p-4">Trường &amp; Chuyên ngành</th><th className="p-4">Trạng thái phát hành</th><th className="p-4">Lượt xem / Tải</th><th className="p-4">Thời gian tải</th><th className="p-4 text-right">Thao tác</th></tr></thead><tbody className="divide-y divide-[#e9edff]">{loading && <tr><td className="p-6 text-center text-[#444653]" colSpan="6">Đang tải dữ liệu...</td></tr>}{!loading && documents.map((item) => <tr className="hover:bg-[#f1f3ff]/50" key={item.id}><td className="p-4"><div className="flex items-center gap-3">{item.thumbnailUrl ? (<img alt="" className="h-12 w-9 shrink-0 rounded border border-[#c4c5d5]/60 object-cover object-top shadow-xs" src={item.thumbnailUrl.startsWith('http') ? item.thumbnailUrl : `${apiOrigin}${item.thumbnailUrl}`} />) : (<span className="grid h-11 w-9 shrink-0 place-items-center rounded bg-[#dce2f7] text-lg text-[#00288e]">▤</span>)}<span><strong className="block max-w-xs truncate text-sm text-[#141b2b]">{item.title}</strong><small className="text-[10px] text-[#444653]">HLS-{item.id.slice(-6)} · {item.fileFormat} · {item.pageCount || '--'} trang</small></span></div></td><td className="p-4"><strong className="block text-[#00288e]">{item.university?.shortName || item.university?.name || 'Chưa cập nhật'}</strong><span className="text-[10px] text-[#444653]">{item.subject?.code || 'Chưa gắn môn'}</span></td><td className="p-4"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${item.status === 'PUBLISHED' ? 'bg-[#d9f7ed] text-[#00563a]' : 'bg-[#fffbeb] text-[#92400e]'}`}>{item.status === 'PUBLISHED' ? 'Đã xuất bản' : item.status === 'PENDING_REVIEW' ? 'Chờ kiểm duyệt' : item.status}</span></td><td className="p-4"><strong>{item.viewCount || 0} / {item.downloadCount || 0}</strong><small className="block text-[10px] text-[#444653]">★ {Number(item.ratingAverage || 0).toFixed(1)}</small></td><td className="p-4 text-[10px] text-[#444653]">{new Date(item.createdAt).toLocaleDateString('vi-VN')}</td><td className="p-4 text-right whitespace-nowrap"><div className="flex items-center justify-end gap-1.5"><a href={`/tai-lieu/${item.id}`} target="_blank" rel="noreferrer" className="rounded-lg bg-[#f1f3ff] p-1.5 text-[#00288e] hover:bg-[#e9edff] transition inline-flex items-center" title="Xem chi tiết tài liệu"><span className="material-symbols-outlined text-[16px]">visibility</span></a><button onClick={() => onEditDocument(item)} className="rounded-lg bg-[#f1f3ff] p-1.5 text-[#0058be] hover:bg-[#e9edff] transition" title="Chỉnh sửa thông tin" type="button"><span className="material-symbols-outlined text-[16px]">edit</span></button><button onClick={() => onDeleteDocument(item.id)} className="rounded-lg bg-slate-100 p-1.5 text-slate-500 hover:bg-red-50 hover:text-red-600 transition" title="Xóa tài liệu" type="button"><span className="material-symbols-outlined text-[16px]">delete</span></button></div></td></tr>)}{!loading && documents.length === 0 && <tr><td className="p-8 text-center text-[#444653]" colSpan="6">Bạn chưa có tài liệu. Hãy tải lên tài liệu đầu tiên.</td></tr>}</tbody></table></div></section>
}
